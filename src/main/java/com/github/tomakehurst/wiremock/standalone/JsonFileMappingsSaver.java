...
public class JsonFileMappingsSaver implements MappingsSaver {
  ...
  public void saveMappings(StubMappings stubMappings) {
    Collection<StubMapping> transientStubs =
        filter(stubMappings.getAll(), new Predicate<StubMapping>() {
    ...
    for (StubMapping mapping : transientStubs) {
      if (mapping.getFilePath() == null) {
        String fileId = idGenerator.generate();
        mapping.setFilePath("saved-mapping-" + fileId + ".json");
      }
      mappingsFileSource.writeTextFile(mapping.getFilePath(), write(mapping));
