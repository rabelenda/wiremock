...
public class MappingsLoaderAcceptanceTest {
  ...
  private void constructWireMock() {
    ...
    final File[] unorderedFiles =
        {new File("src/test/resources/test-requests/401-example.json"),
    ...
    context.checking(new Expectations() {
      {
        allowing(unorderedSourceFolder).exists();
        will(returnValue(true));
        allowing(unorderedSourceFolder).isDirectory();
        will(returnValue(true));
        allowing(unorderedSourceFolder).getPath();
        will(returnValue("src/test/resources/test-requests"));
        allowing(unorderedSourceFolder).listFiles();
        will(onConsecutiveCalls(returnValue(unorderedFiles), returnValue(new File[0])));
      }
    });
    MappingsLoader mappingsLoader =
        new JsonFileMappingsLoader(new SingleRootFileSource(unorderedSourceFolder));
